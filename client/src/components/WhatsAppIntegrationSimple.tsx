...
const WhatsAppIntegration: React.FC<WhatsAppIntegrationProps> = ({ 
  ...
  const handleConnect = async () => {
    ...
    try {
      const response = await makeRequest(`/api/whatsapp/connect/${chatbotId}`, {
        method: 'POST'
      });
  ...
  const handleDisconnect = async () => {
    ...
    try {
      const response = await makeRequest(`/api/whatsapp/disconnect/${chatbotId}`, {
        method: 'POST'
      });
        ...
        <Card>
          <CardContent className="text-center p-6">
            <div className="border-2 border-blue-200 rounded-lg bg-blue-50 p-6">
              ...
              <h3 className="text-lg font-semibold mb-2 text-blue-800">Generando código QR...</h3>
