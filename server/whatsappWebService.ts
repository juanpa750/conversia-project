...
class WhatsAppWebService extends EventEmitter {
  ...
  private async waitForQRCode(page: Page): Promise<string> {
    try {
      ...
      // Intentar múltiples métodos para encontrar el QR
      let qrCode = null;

      // Método 1: Selectores actualizados de WhatsApp Web
      const qrSelectors = [
        '[data-ref="qr-code"]',
        '[data-testid="qr-code"]', 
      ...
      for (const selector of qrSelectors) {
        try {
          ...
          if (qrElement) {
            ...
            qrCode = await qrElement.screenshot({ encoding: "base64" });
            return `data:image/png;base64,${qrCode}`;
          }
        } catch (e) {
          console.log(`❌ Selector ${selector} no funcionó:`, e.message);
        }
      }

      // Método 2: Esperar más tiempo y buscar cualquier canvas
      console.log("🔄 Buscando canvas después de espera adicional...");
      ...
      if (canvasElements.length > 0) {
        ...
        for (let i = 0; i < canvasElements.length; i++) {
          try {
            ...
            // Verificar que no sea un canvas vacío (mínimo 1KB)
            if (screenshot && screenshot.length > 1000) {
              console.log(`✅ QR generado desde canvas ${i}`);
              return `data:image/png;base64,${screenshot}`;
            }
          } catch (e) {
            console.log(`❌ Error con canvas ${i}:`, e.message);
          }
        }
      }

      // Método 3: Screenshot de toda la página como fallback
      console.log("📸 Generando screenshot de toda la página como fallback...");
