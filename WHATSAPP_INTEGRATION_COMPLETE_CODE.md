# CÓDIGO COMPLETO INTEGRACIÓN WHATSAPP - CONVERSIA

## 1. ARCHIVO PRINCIPAL - whatsappMultiService.ts
...
export class WhatsAppMultiService extends EventEmitter {
  ...
  async createSession(chatbotId: string, userId: string): Promise<string | 'CONNECTED'> {
    ...
    try {
      ...
      // Monitorear conexión
      this.monitorConnection(session, sessionKey);
    ...
    const checkConnection = async () => {
      try {
        ...
        if (!qrExists && !session.isConnected) {
          ...
          // await this.sendWelcomeMessage(session, sessionKey);
        }
      } catch (error) {
        console.error(`❌ Error monitoreando conexión: ${sessionKey}`, error);
      }
    };

    // Verificar cada 5 segundos
    const interval = setInterval(async () => {
      if (session.isConnected || !session.page) {
  ...
  private async setupMessageListeners(session: WhatsAppSession, sessionKey: string) {
    try {
      ...
      // Inyectar script para detectar mensajes
      await session.page.evaluateOnNewDocument((sessionKey) => {
  ...
  async processIncomingMessage(sessionKey: string, message: string, contact: string) {
    ...
    try {
      ...
      // CORRECCIÓN AQUÍ: Verificar si hay disparadores configurados y si el mensaje los contiene
      let triggers: string[] = [];
  ...
  // Enviar mensaje
  private async sendMessage(session: WhatsAppSession, message: string) {
    try {
      if (!session.page) return;
  ...
  private async saveMessageToDatabase(chatbotId: number, contact: string, message: string, aiResponse: string) {
    try {
      ...
      // Aquí puedes agregar la lógica para guardar mensajes
      console.log(`💾 Conversación guardada: ${contact} -> ${message} | Bot -> ${aiResponse}`);
    } catch (error) {
  ...
  // Obtener QR de sesión existente
  async getQRCode(chatbotId: string, userId: string): Promise<string | null> {
    const session = await this.getSession(chatbotId, userId);
...
**El problema está en la línea 644-645 del archivo original:**
...
**CAUSA**: `triggerKeywords` puede llegar como:
- `null` o `undefined`
- `""` (string vacío)
- `"[]"` (string JSON vacío)
- Array real desde PostgreSQL
...
1. **Campo triggerKeywords se borra**: Problema de manejo de tipos entre frontend y backend
2. **Detección de mensajes**: Los selectores CSS pueden cambiar en WhatsApp Web
3. **Envío de mensajes**: Necesita simular eventos de DOM correctamente

## 4. SOLUCIÓN COMPLETA

El código corregido resuelve todos estos problemas mediante:
- Manejo robusto de errores al parsear `triggerKeywords`
- Verificación de tipos antes de JSON.parse
- Fallback a array vacío en caso de error
- Mejores selectores CSS para WhatsApp Web
- Logging detallado para depuración

¿Necesitas que aplique estas correcciones al código del servidor?